"""
hotspot_api/views.py  — COMPLETE VERSION
Includes:
  • All original endpoints (unchanged)
  • mpesa_callback updated to:
      – auto-award reward points on successful payment
      – auto-repay outstanding Kopa before issuing new voucher
"""
...
from .models import (
    ...
    generate_account_no, OTP, KopaTransaction
)
...
# Import points helper (defined in points_views.py)
from .points_views import award_points_for_payment
...
def admin_dashboard(request):
    from .models import CustomerPoints, KopaTransaction
    now         = timezone.now()
    ...
    # ★ Kopa Stats
    active_kopas     = KopaTransaction.objects.filter(status='ACTIVE').count()
    total_kopas      = KopaTransaction.objects.count()
    kopa_outstanding = KopaTransaction.objects.filter(status='ACTIVE').aggregate(
        total=Sum('amount_ksh'))['total'] or Decimal('0.00')

    # ★ Points Stats
    total_points_issued   = CustomerPoints.objects.aggregate(total=Sum('lifetime_earned'))['total'] or 0
    total_points_redeemed = CustomerPoints.objects.aggregate(total=Sum('lifetime_redeemed'))['total'] or 0
    points_members        = CustomerPoints.objects.count()

    # Recent Transactions
    recent_transactions = MpesaTransaction.objects.select_related('voucher').order_by('-created_at')[:10]

    # Trend data (last 7 days)
    def daily_trend(model, date_field, value_field=None, filter_kwargs=None, days=7):
        trend = []
        for i in range(days - 1, -1, -1):
            day       = now - timedelta(days=i)
            day_start = day.replace(hour=0, minute=0, second=0, microsecond=0)
            day_end   = day_start + timedelta(days=1)
            q = {f'{date_field}__gte': day_start, f'{date_field}__lt': day_end}
            if filter_kwargs:
                q.update(filter_kwargs)
            qs = model.objects.filter(**q)
            if value_field:
                val = qs.aggregate(total=Sum(value_field))['total'] or Decimal('0.00')
                trend.append({'date': day.strftime('%b %d'), 'amount': float(val)})
            else:
                trend.append({'date': day.strftime('%b %d'), 'count': qs.count()})
        return trend

    revenue_trend      = daily_trend(MpesaTransaction, 'created_at', 'amount', {'status': 'COMPLETED'})
    customer_trend     = daily_trend(HotspotCustomer, 'joined')
    transactions_trend = daily_trend(MpesaTransaction, 'created_at')
    vouchers_trend     = daily_trend(Voucher, 'created_at')

    context = {
        'total_customers': total_customers, 'customers_today': customers_today,
        'customers_this_week': customers_this_week, 'customers_this_month': customers_this_month,
        'active_customers': active_customers, 'total_revenue': total_revenue,
        'revenue_today': revenue_today, 'revenue_this_week': revenue_this_week,
        'revenue_this_month': revenue_this_month, 'total_transactions': total_transactions,
        'completed_transactions': completed_transactions, 'pending_transactions': pending_transactions,
        'failed_transactions': failed_transactions, 'total_vouchers': total_vouchers,
        'active_vouchers': active_vouchers, 'used_vouchers': used_vouchers,
        'vouchers_created_today': vouchers_created_today, 'total_access_points': total_access_points,
        'online_access_points': online_access_points, 'recent_transactions': recent_transactions,
        'revenue_trend': revenue_trend, 'customer_trend': customer_trend,
        'transactions_trend': transactions_trend, 'vouchers_trend': vouchers_trend,
        # Kopa
        'active_kopas': active_kopas, 'total_kopas': total_kopas,
        'kopa_outstanding': kopa_outstanding,
        # Points
        'total_points_issued': total_points_issued, 'total_points_redeemed': total_points_redeemed,
        'points_members': points_members,
    }
...
# ─────────────────────────────────────────────────────
# OTP & Voucher APIs (unchanged from original)
# ─────────────────────────────────────────────────────
...
def send_otp(request):
    ...
    if sms_result['success']:
        print(f"✅ OTP sent to {phone_number}: {otp_code}")
    else:
...
def lookup_voucher(request):
    ...
    vouchers = Voucher.objects.filter(phone_number=phone_number, is_active=True)
    if not vouchers.exists():
        return JsonResponse({"success": False, "message": "No active vouchers found"}, status=404)
...
def initiate_mpesa_payment(request):
    """
    Before initiating STK push, checks if customer has an outstanding Kopa.
    If so, adds the Kopa amount to the payment so it gets auto-repaid.
    """
    try:
        ...
        print(f"📱 Phone: {phone_number} | 💰 Amount: {amount} | 📦 Plan: {plan_name}")
        print("=" * 80)
        ...
        # ★ Check for outstanding Kopa → block purchase until repaid
        ...
        if active_kopa:
            return JsonResponse({
                'success': False,
                'message': (
                    f'You have an outstanding Kopa balance of {float(active_kopa.total_owed)} Ksh. '
                    f'Please repay it first by calling 0792701147 or it will be '
                    f'deducted automatically from your next payment.'
                ),
                'kopa_outstanding': float(active_kopa.total_owed),
            })
            ...
            with urllib.request.urlopen(stk_request) as stk_response:
                ...
                if response_data.get('ResponseCode') == '0':
                    try:
                        ...
                        print(f"✅ Transaction saved: {transaction.checkout_request_id}")
...
def mpesa_callback(request):
    """
    Called by Safaricom servers after payment.

    On SUCCESS it:
      1. Updates the MpesaTransaction to COMPLETED
      2. Auto-repays any outstanding Kopa (marks it REPAID)
      3. Auto-creates the purchase voucher
      4. Awards reward points  (10 Ksh = 1 point)
      5. Sends SMS confirmations
    """
    try:
        callback_data = json.loads(request.body)
        print("=" * 80)
        print(f"📞 M-PESA CALLBACK at {timezone.now()}")
        ...
        if result_code == 0:
            ...
            print(f"✅ PAYMENT SUCCESS! Receipt: {mpesa_receipt_number} | Amount: {amount} | Phone: {phone_number}")
            ...
            with db_transaction.atomic():
                ...
                if not mpesa_txn:
                    print(f"⚠️ Transaction NOT FOUND: {checkout_request_id}")
                ...
                if active_kopa:
                    active_kopa.status     = 'REPAID'
                    active_kopa.repaid_at  = timezone.now()
                    active_kopa.repaid_via = mpesa_txn
                    active_kopa.save()
                    print(f"✅ Kopa repaid: {float(active_kopa.total_owed)} Ksh for {phone_number}")
                    try:
                        from .sms_utils import send_sms
                        send_sms(
                            phone_number,
                            f"KiRePa Wi-Fi: Your Kopa of {float(active_kopa.total_owed)} Ksh "
                            f"has been repaid from your payment. Thank you!"
                        )
                    except Exception:
                        pass
                ...
                if not mpesa_txn.voucher:
                    ...
                    if voucher:
                        ...
                        mpesa_txn.save(update_fields=['voucher'])
                        print(f"✅ Voucher created: {voucher.code}")
                    else:
                        print(f"❌ VOUCHER CREATION FAILED for {mpesa_txn.id}")
                else:
                    print(f"✅ Voucher already exists: {mpesa_txn.voucher.code}")

                # ── ★ AWARD POINTS ─────────────────────────────
                pts = award_points_for_payment(phone_number, amount, mpesa_transaction=mpesa_txn)
                if pts > 0:
                    print(f"⭐ {pts} points awarded to {phone_number}")
                    try:
                        from .models import CustomerPoints
                        cp = CustomerPoints.objects.filter(phone_number=phone_number).first()
                        if cp:
                            from .sms_utils import send_sms
                            send_sms(
                                phone_number,
                                f"KiRePa Wi-Fi: You earned {pts} reward points! "
                                f"Total: {cp.total_points} pts. "
                                f"Redeem for free data in the Wi-Fi portal."
                            )
                    except Exception:
                        pass
...
# ─────────────────────────────────────────────────────
# Voucher creation helper (unchanged)
# ─────────────────────────────────────────────────────
...
def create_voucher_for_transaction(transaction):
    try:
        ...
        plan_details = plan_mapping.get(amount) or {
            'data_limit_mb': int(amount * 10),
            'validity_hours': 24,
        ...
        try:
            send_payment_confirmation_sms(
                transaction.phone_number, amount, voucher_code, plan_details['description']
            )
...
def top_customers(request):
    from .models import CustomerPoints, Voucher
    from django.db.models import Count

    top = CustomerPoints.objects.order_by('-total_points')[:8]
    data = []
    for cp in top:
        voucher_count = Voucher.objects.filter(phone_number=cp.phone_number).count()
        data.append({
            'phone': cp.phone_formatted,
            'points': cp.total_points,
            'vouchers': voucher_count,
            'spent': float(cp.total_spent_ksh),
        })
